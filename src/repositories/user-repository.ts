...
export default class UserRepository {
  ...
  static async findUserByID(userID: uuid): Promise<IUser | null> {
    const query =
      'SELECT id, username, email, first_name as "firstName", last_name as "lastName", squad, is_admin as "isAdmin" FROM "User" WHERE id = $1';
  ...
  static async findAllUsersBySquad(team_id: uuid): Promise<IUser[]> {
    const query = 'SELECT * FROM "User" WHERE squad = $1';
    const { rows } = await dBConnection.query(query, [team_id]);
